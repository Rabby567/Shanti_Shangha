...
if($action==='assign-donor'&&$_SERVER['REQUEST_METHOD']==='POST'){
  ...
  try{$q=$pdo->prepare('INSERT INTO blood_request_donors(request_id,donor_id,confirmed_at) VALUES(?,?,CURRENT_TIMESTAMP)');$q->execute([$requestId,$donorId]);send_json(['success'=>true,'message'=>'রক্তদাতা নির্বাচিত হয়েছে।']);}catch(PDOException $e){send_json(['success'=>false,'message'=>'এই donor ইতিমধ্যে নির্বাচিত হয়েছে।'],409);}
}
if($action==='remove-assigned-donor'&&$_SERVER['REQUEST_METHOD']==='POST'){
  $p=blood_payload();$requestId=(int)($p['request_id']??0);$donorId=(int)($p['donor_id']??0);$q=$pdo->prepare('DELETE FROM blood_request_donors WHERE request_id=? AND donor_id=? AND donated_at IS NULL');$q->execute([$requestId,$donorId]);send_json(['success'=>true,'message'=>'নির্বাচিত donor সরানো হয়েছে।']);
}
if($action==='fulfill-request'&&$_SERVER['REQUEST_METHOD']==='POST'){
  ...
  $st=$pdo->prepare('UPDATE blood_requests SET status="fulfilled" WHERE id=?');$st->execute([$requestId]);$pdo->commit();send_json(['success'=>true,'message'=>'রক্তদান সম্পন্ন হিসেবে সংরক্ষণ করা হয়েছে।']);
}
...
if($action==='request-status'&&$_SERVER['REQUEST_METHOD']==='POST'){$p=blood_payload();$id=(int)($p['id']??0);$st=(string)($p['status']??'');if(!in_array($st,['pending','approved','fulfilled','cancelled'],true))send_json(['success'=>false,'message'=>'Invalid status.'],422);$q=$pdo->prepare('UPDATE blood_requests SET status=? WHERE id=?');$q->execute([$st,$id]);send_json(['success'=>true,'message'=>'রক্তের আবেদনের status আপডেট হয়েছে।']);}
 ...
 if($action==='delete-request'&&$_SERVER['REQUEST_METHOD']==='POST'){$p=blood_payload();$q=$pdo->prepare('DELETE FROM blood_requests WHERE id=?');$q->execute([(int)($p['id']??0)]);send_json(['success'=>true,'message'=>'রক্তের আবেদন মুছে ফেলা হয়েছে।']);}
 if($action==='delete-donor'&&$_SERVER['REQUEST_METHOD']==='POST'){$p=blood_payload();$q=$pdo->prepare('DELETE FROM blood_donors WHERE id=?');$q->execute([(int)($p['id']??0)]);send_json(['success'=>true,'message'=>'রক্তদাতার রেকর্ড মুছে ফেলা হয়েছে।']);}
